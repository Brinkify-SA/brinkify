...
export default function FeedPage() {
  ...
  const [sortBy, setSortBy] = useState<"recent" | "trending" | "top-rated">(
    "recent"
  );
    ...
    const fetchJobs = async () => {
      try {
        ...
        const mapped: FeedPost[] = data.map((j: any) => ({
          ...
          tags: [],
          price:
            j.min_budget || j.max_budget
              ? `R${j.min_budget || ""}${
                  j.max_budget ? " - R" + j.max_budget : ""
                }`
              : "Price on request",
          completionTime: "",
          likes: 0,
  ...
  const filteredPosts = posts
    ...
    .sort((a, b) => {
      ...
      const tierDiff =
        getSubscriptionTier(b.workerId) - getSubscriptionTier(a.workerId);
      ...
      if (sortBy === "top-rated")
        return (
          b.worker.rating * b.worker.reviews -
          a.worker.rating * a.worker.reviews
        );
  ...
  const contactWorker = (post: FeedPost) => {
    ...
    const convId = `conv-${Date.now()}-${Math.random()
      .toString(36)
      .substring(7)}`;
                      ...
                      {post.verified && (
                        <div
                          className={`absolute top-3 left-3 px-2 py-1 rounded-full text-white text-xs font-medium flex items-center gap-1 ${
                            WORKER_SUBSCRIPTIONS[post.workerId] === "elite"
                              ? "bg-green-500"
                              : WORKER_SUBSCRIPTIONS[post.workerId] ===
                                "premium"
                              ? "bg-blue-500"
                              : "bg-orange-500"
                          }`}
                        >
                          ...
                          {WORKER_SUBSCRIPTIONS[post.workerId] === "elite"
                            ? "Verified Elite"
                            : WORKER_SUBSCRIPTIONS[post.workerId] === "premium"
                            ? "Verified Pro"
                            : "Verified"}
                        </div>
                      )}
